...
class Environment(gym.Env):
        ...
        if action == self.pa.num_machine*self.pa.job_slots: #no action
            ...
            if done == True: # If I'm teminating the episode
                if self.state.job_slot.slot != [None] * self.pa.job_slots: # not all job are assigned
                    reward -= 0 # malus for terminating the episode without shceduling all jobs.
                else:
                    reward += 0 # bonus for corrcting terminate the episode by assigning all the jobs.
    ...
    def action_masks(self):
        ''' 
            Output: 
            vector of 0 and 1 of same size of job_slot (0 means that I cannot execute the job in that index of the job slot)
            '''

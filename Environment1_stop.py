...
class Environment(gym.Env):
    ...
    Episode termination:
        ...
        There is early criteria of termination when the agent choose an empty job slot more than 10 times or when choose the no action (wait and not doing anything) when there were some other action possible.
    """

    def __init__(self,pa, day_job_jobcode, jobcode_machine_op_t, turn_abilities):
        super(Environment, self).__init__()
        '''
        This function can be used for configuring the selfironment
        INPUT:
        ...
        self.num_null_slot = 0 # is the number of consecutive times in which he select an empty job slot
            ...
            for job_code in state.job_slot.slot:
                ...
                if job_code == None: # I have no job in that position of the job_slot
                    ...
                    if machine_code in job.suitable_machine: # If I can execute that job on that machine, i put the info of the job
                        ...
                        machine_job.extend([job.job_time[index][0], job.operators[index][0]]) 
    ...
    def step(self, action):
        """
        Take an action in the selfironment and observe the next transition.
        ...
        if action == self.pa.num_machine*self.pa.job_slots: #no action
            ...
            if job_code == -9: # I have selected no job so it's like don't do any action
                ...
                return(self.no_job_action(self.num_null_slot))
            else:
                self.num_null_slot = 0
    ...
    def reset(self):
        """
        Reset the selfironment to execute a new episode.
    ...
    def render(self):
        """
        Render a state of the selfironment.
    ...
    def get_reward(self, state, action):
                    ...
                    for machine_code in machine_codes:
                        ...
                        if machine.available == 1:
                            ...
                            if sum([state.operators[operator].available for operator in op_code]) >= num_op: 
                                return -1 # There is a possible action to execute but the system has preferred to wait (no good)
                            else:
        ...
        # I'M TAKING AN ACTION
            ...
            # CALCULATE THE REWARD WHEN I EXECUTE A POSSIBLE ACTION
                ...
                difference = max([(machine.time_scheduled + time) - (state.machines[mach].time_scheduled + job_mach_time) for mach, job_mach_time in zip(possible_machine, job.job_time)])
                ...
                diff_time = max([time-other_time for other_time in job.job_time]) 
                reward -= diff_time
                reward = float(reward)
    ...
    def _get_next_state(self, old_state, action):
        ...
        """
        state = copy.deepcopy(old_state) # I don't want to modify the state of the selfironment but create a new state instance which represents how the new state could be.
        ...
        if machine.available == 0:
            ...
            if sum([state.operators[operator].available for operator in op_code]) >= num_op:
                ...
                job.actual_time = float(job.job_time[index])
                job.num_op = num_op
    ...
    def no_action_step(self):
        '''This function will be used in two cases: - The NN choose the no action - it is chosen an empty jobslot.
        ...
        '''In case the selected action is the No action we will return the self as it is now.'''
    ...
    def no_job_action(self, num_null_action):

        obs = self.observe(self.state)
        reward = -15
        info = {}
        if num_null_action >= 10: # if I have more than 10 consecutively action in which I select an empty job slot I stop the alg 
            done = True
        else:
            done = self.finish == True
